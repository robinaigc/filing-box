...
const allowedStatuses = ["success", "empty", "failed", "missing", "with_reports"] as const;
...
if (statusArg && !allowedStatuses.includes(statusArg as CoverageStatus)) {
  throw new Error("--status must be one of success, empty, failed, missing, with_reports.");
}
...
async function fetchAllCompanies(): Promise<CompanyRow[]> {
  ...
  for (let from = 0; ; from += pageSize) {
    const { data, error } = await supabase
      .from("companies")
      .select("id, symbol, display_name, cik")
      .eq("market", "US")
...
function printSummary(rows: CoverageRow[], secReportCount: number) {
